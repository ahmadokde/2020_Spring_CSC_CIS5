...
/* 
 * File:   LabExercise8b.cpp
 * Author: ahmad okde
 *
 * Created on May the 7th be with you, 2020
 */
...
int main(int argc, char** argv) {
  ...
  long long int seatKms[DATA_SIZE];
  long long int incidents[DATA_SIZE];
  long long int fatalAccidents[DATA_SIZE];
  long long int fatalities[DATA_SIZE];
 ...
 searchBlock1:
  ...
  cout << "--------------------------------------------------------\n";
  // Sort the arrays
  sortArrays(airlineName, seatKms, incidents, fatalAccidents, fatalities, rowCount);
 ...
 searchBlock2:
  // Binary search based on seatKMs
  {
    ...
    for (int index = 0; index < listSize; index++) {
      ...
      position = binarySearchR(seatKms,  0, rowCount-1, searchList[index]);
